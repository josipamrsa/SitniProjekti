...
namespace Pmfst_GameSDK
{
    ...
    /* 
        U klasi Sprite je dodatno napravljeno svojstvo razine jer ju koriste sve klase, svojstva Y i X su postavljena 
        tako da se mogu overridati u ostalim klasama.

        Klasa Zeko nasljedjuje samo od klase Sprite te sadrzi override svojstva X gdje zaustavlja lika da ode preko
        ruba pozornice. Sadrzi dodatno jos 2 svojstva - brzina i bodovi. Ako se razina poveca, s tim se dodatno
        povecavaju vrijednosti i ova dva svojstva.

        Klasa Likovi je apstraktna klasa iz koje nasljedjuju klase Mrkva i Zamka. Sadrzi vrijednost, vrijeme i 
        brzinu likova te nasljedjuje od klase Sprite. 

        Klasa Mrkva ima override svojstva Vrijeme na nacin da baca gresku iz klase VrijemeException (nasljedjiva
        od klase Exception) ukoliko vrijeme postane negativno. To se vraca u glavni program gdje se iznimka hvata
        i vrijednost se postavlja na fiksnu vrijednost.

        Klasa Zamka u konstruktoru ima definirano samo vrijeme.
         
     */
   
    public class Zeko : Sprite
    {
        public override int X
        {
            ...
            set
            {
                if (value > GameOptions.RightEdge-150)
                {
                    this.x = GameOptions.RightEdge - 150;

                }

                else if (value < GameOptions.LeftEdge + 50)
                {
                    this.x = GameOptions.LeftEdge + 50;
                }
        ...
        public Zeko(string p, int x, int y)
            : base(p,x,y)
        {
            this.Brzina = 15;
        }
    ...
    {
        public Likovi(string p, int x, int y)
            : base(p,x,y)
        { }

        private int _vrijednost;
        private int _brzina;
        protected double _vrijeme;
    ...
    public class VrijemeException : Exception
    {
        public VrijemeException() : base("Negativna vrijednost")
        {

        }
    }

    public class Mrkva : Likovi
    {        
        public Mrkva(string p, int x, int y)
            : base(p,x,y)
        {
            this.Vrijednost = 1;
            this.Brzina = 25;
        ...
        public override double Vrijeme
        {
            get
            {
                return _vrijeme;
    ...
    public class Zamka : Likovi
    {
        public Zamka(string p, int x, int y)
            : base(p,x,y)
        {
            this.Vrijeme = 0.8;
        }
